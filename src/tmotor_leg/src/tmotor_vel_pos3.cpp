...
ros::Subscriber Imu_sub, Joy_sub, yawAngle_low_sub, yawAngle_high_sub;
...
// get yaw angle from lower platform
geometry_msgs::PolygonStamped yaw_angle_low, yaw_angle_high;
double P_geo;
...
void yawAngle_lowCB(const geometry_msgs::PolygonStamped::ConstPtr& yaw_angle_low_msg){
	yaw_angle_low = *yaw_angle_low_msg;
	tmotor[0].asoc_angle = yaw_angle_low.polygon.points[0].x * PI / 180;
	tmotor[3].asoc_angle = yaw_angle_low.polygon.points[2].x * PI / 180;
	tmotor[0].asoc_length = -cos(tmotor[0].asoc_angle) * 0.085; //asoc offset distance = 0.085m;
	tmotor[3].asoc_length = cos(tmotor[3].asoc_angle) * 0.085;

}

void yawAngle_highCB(const geometry_msgs::PolygonStamped::ConstPtr& yaw_angle_high_msg){
	yaw_angle_high = *yaw_angle_high_msg;
	tmotor[1].asoc_angle = yaw_angle_high.polygon.points[0].x * PI / 180;
	tmotor[2].asoc_angle = yaw_angle_high.polygon.points[2].x * PI / 180;
	tmotor[1].asoc_length = sin(tmotor[1].asoc_angle) * 0.085;
	tmotor[2].asoc_length = -sin(tmotor[2].asoc_angle) * 0.085;
...
void imuCB(const geometry_msgs::PolygonStamped::ConstPtr &sensorMsg)
{
	...
	e_roll = roll_set - roll;
	e_pitch = pitch_set - pitch;
	...
	// 	tmotor[i].t_pd = K_S * (tmotor[i].pos_zero-tmotor[i].pos_now); //20220907 - D_S * tmotor[i].vel_now
    // }
...
//velocity controller
	...
	// tmotor[1].t_pd =0.0;
    // tmotor[1].t_pd = (P_pit * e_pitch + D_pit * (e_pitch - lastPitch)) + I_pit * pitch_sum + P_pos * errorPosition_now[1] + D_pos * (errorPosition_now[1] - errorPosition_last[1]);

//torque controller
	// tmotor[2].t_pd = P_pit * e_pitch + D_pit * (e_pitch - lastPitch) + I_pit * pitch_sum + P_pos * errorPosition_now[2] + D_pos * (errorPosition_now[2] - errorPosition_last[2]);
    // tmotor[0].t_pd = -(P_rol * e_roll + D_rol * (e_roll - lastRoll)) + I_rol * roll_sum + P_pos * errorPosition_now[0] + D_pos * (errorPosition_now[0] - errorPosition_last[0]);
    // tmotor[1].t_pd = -(P_pit * e_pitch + D_pit * (e_pitch - lastPitch)) + I_pit * pitch_sum + P_pos * errorPosition_now[1] + D_pos * (errorPosition_now[1] - errorPosition_last[1]);
    // tmotor[3].t_pd = P_rol * e_roll + D_rol * (e_roll - lastRoll) + I_rol * roll_sum + P_pos * errorPosition_now[3] + D_pos * (errorPosition_now[3] - errorPosition_last[3]);

//torque controller using geometry mapping
//Tmotor [1]&[2] control pitch angle
//Tmotor [0]&[3] control roll angle
    tmotor[1].pid_weight = -(P_pit * e_pitch + D_pit * (e_pitch - lastPitch) + I_pit * pitch_sum);
	tmotor[2].pid_weight = P_pit * e_pitch + D_pit * (e_pitch - lastPitch) + I_pit * pitch_sum;
    tmotor[0].pid_weight = -(P_rol * e_roll + D_rol * (e_roll - lastRoll) + I_rol * roll_sum);	
    tmotor[3].pid_weight = P_rol * e_roll + D_rol * (e_roll - lastRoll) + I_rol * roll_sum;

	tmotor[0].length = tmotor[0].link_length + tmotor[0].asoc_length;
	tmotor[1].length = tmotor[1].link_length + tmotor[1].asoc_length;
	tmotor[2].length = tmotor[2].link_length + tmotor[2].asoc_length;
	tmotor[3].length = tmotor[3].link_length + tmotor[3].asoc_length;

	tmotor[2].vel_pd = P_geo * (tmotor[2].length / tmotor[1].length) * tmotor[2].pid_weight;
	tmotor[1].vel_pd = P_geo * (tmotor[1].length / tmotor[2].length) * tmotor[1].pid_weight;
	tmotor[0].vel_pd = P_geo * (tmotor[0].length / tmotor[3].length) * tmotor[0].pid_weight;
	tmotor[3].vel_pd = P_geo * (tmotor[3].length / tmotor[0].length) * tmotor[3].pid_weight;

//test 1: one Tmotor to control attitude
	// tmotor[1].t_pd = 0;
    lastRoll = e_roll;
	lastPitch = e_pitch;
	roll_sum += lastRoll;
...
void rxThread(int s)
{
	...
	for (i = 0;; i++)
	{
		...
		tmotor[ID].pitch_angle = (0.25 * 180 / PI * tmotor[ID].pos_now + 73 ) * PI / 180;
		tmotor[ID].link_length = 0.178 + 0.18 * sin(tmotor[ID].pitch_angle);
...
void motorParaSet(int id)
{
	...
	case 1:
		// // 20220907
		// tmotor[id].t_des = tmotor_torque + tmotor[id].t_pd;
		// tmotor[id].pos_des = 0;
		// tmotor[id].vel_des = 0;
		// tmotor[id].kp = 0;
		// tmotor[id].kd = 0;
		...
		//todo
		// for (int i = 0; i < 3; i++) {
		// tmotor[i].t_des = tmotor_torque;
		// tmotor[i].pos_des = tmotor[id].pos_zero ;
		// tmotor[i].vel_des = 0;
		// tmotor[i].kp = K_S;
		// tmotor[i].kd = 0;
		// }
		// tmotor[3].t_des = tmotor_torque;
		// tmotor[3].pos_des = tmotor[id].pos_zero + axes_lt;
		// tmotor[3].vel_des = 0;
		// tmotor[3].kp = K_S;
		// tmotor[3].kd = 0;		
		break;
	...
	case 2:
		...
		// tmotor[id].t_des = tmotor[id].t_pd;
		// tmotor[id].pos_des = 0;
		// tmotor[id].vel_des = 0;
		// tmotor[id].kp = 0;
		// tmotor[id].kd = 0;
		...
		// for (int i = 0; i < 3; i++) {
		// tmotor[i].t_des = tmotor_torque;
		// tmotor[i].pos_des = tmotor[id].pos_zero + axes_lt;
		// tmotor[i].vel_des = 0;
		// tmotor[i].kp = K_S;
		// tmotor[i].kd = 0;
		// }
		// tmotor[3].t_des = tmotor_torque;
		// tmotor[3].pos_des = tmotor[id].pos_zero ;
		// tmotor[3].vel_des = 0;
		// tmotor[3].kp = K_S;
		// tmotor[3].kd = 0;
...
void printTmotorInfo(int id)
{
	...
	ROS_INFO("\nflag[%d,%d,%d,%d] \npos_now is [%.2f,%.2f,%.2f,%.2f]\npos_des is [%.2f,%.2f,%.2f,%.2f] \nvel_des is [%.3f,%.3f,%.3f,%.3f] \nt_now is [%.2f,%.2f,%.2f,%.2f] \npos_zero is [%.2f,%.2f,%.2f,%.2f] \nt_des is [%.2f,%.2f,%.2f,%.2f] \nstop_flag:%d\n------------\n",
			 ...
			 Stop_flag);
	ROS_INFO("\n e_roll and e_pitch is [%.2f,%.2f] \nroll_set and pitch_set is [%.2f,%.2f]\nasoc_length is [%f,%f,%f,%f] \nlink_length is [%f,%f,%f,%f] \nlength is [%f,%f,%f,%f]", 
	e_roll, e_pitch, roll_set, pitch_set,
	tmotor[0].asoc_length, tmotor[1].asoc_length, tmotor[2].asoc_length, tmotor[3].asoc_length,
	tmotor[0].link_length,tmotor[1].link_length,tmotor[2].link_length,tmotor[3].link_length,
	tmotor[0].length,tmotor[1].length,tmotor[2].length,tmotor[3].length);
...
int main(int argc, char **argv)
{
	...
	//imu_rpy0指腿上imu的值
    Imu_sub = n.subscribe("/imu_rpy1", 1, imuCB);
	Joy_sub = n.subscribe("/joy", 1, joyCB);
	yawAngle_low_sub = n.subscribe("leg_angle_low", 1, yawAngle_lowCB);
	yawAngle_high_sub = n.subscribe("leg_angle_high", 1, yawAngle_highCB);
