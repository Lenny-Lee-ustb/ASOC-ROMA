...
void flagTest1()
{
	...
	if (tmotor.flag == 1)
	{
		if (abs(tmotor.pos_zero - tmotor.pos_now) < 0.05)
		{
			//调零完毕
			tmotor.flag = 2;
		}
	...
	//距离相对零点0.5范围内力矩变大使其更接近原点
	if ((tmotor.flag == 2) && (abs(tmotor.pos_zero - tmotor.pos_now) < 0.5))
	{
		tmotor.flag = 3;
	}

	if ((tmotor.flag == 3) && (abs(tmotor.pos_zero - tmotor.pos_now) >= 0.5))
	{
		tmotor.flag = 2;
	}
}
...
void velTest(struct can_frame &frame)
{
	...
	{
		f_t = -(tmotor.pos_now - tmotor.pos_zero) * 0.06;
		f_p = 0;
		f_v = 0;
		f_kd = 0;
		f_kp = 0;
	...
	{
		f_t = -(tmotor.pos_now - tmotor.pos_zero) * 0.4;
		f_p = 0;
		f_v = 0;
		f_kd = 0;
		f_kp = 0;
