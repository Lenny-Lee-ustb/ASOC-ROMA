...
struct Tmotor
{
	...
	float t_now;   //当前力矩（电流？）
	float t_des;   //前馈力矩（电流？）
	float t_pd;  //torque controller
	float t_ff;  //feedforward torque 


// for geometry mapping use
	float kp;
	float kd;
	float pid_weight;
	float asoc_angle;
	float asoc_length;
	float geometry_mapping;
	float pitch_angle; //= (0.25 * 180 / PI * tmotor[ID].pos_now + 73 ) * PI / 180
	float link_length; //= 0.178 + 0.18 * sin(tmotor[ID].pitch_angle)
	float length; //= link_length + asoc_length
...
// float Tmotor::getPitchAngle() {
// 	return (15.52 * pos_now + 73 ) * PI / 180;
// }

// float Tmotor::getLinkLength() { 
// 	float pitchAngle = (15.52 * pos_now + 73 ) * PI / 180;
// 	return 0.178 + 0.18 * sin(pitchAngle);
// }
