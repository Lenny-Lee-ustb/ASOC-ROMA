...
void imuCB(const geometry_msgs::PolygonStamped::ConstPtr &sensorMsg)
{
    ...
    tmotor[2].vel_pd = P_pit * e_pitch + D_pit * (e_pitch - lastPitch) + I_pit * pitch_sum ;
    tmotor[0].vel_pd = -(P_rol * e_roll + D_rol * (e_roll - lastRoll)) + I_rol * roll_sum ;
    tmotor[1].vel_pd = -(P_pit * e_pitch + D_pit * (e_pitch - lastPitch)) + I_pit * pitch_sum ;
    tmotor[3].vel_pd = P_rol * e_roll + D_rol * (e_roll - lastRoll) + I_rol * roll_sum ;



    // tmotor[2].vel_pd = P_pit * pitch + D_pit * (pitch - lastPitch) + I_pit * pitch_sum + P_pos * errorPosition_now[2] + D_pos * (errorPosition_now[2] - errorPosition_last[2]);
    // tmotor[0].vel_pd = -(P_rol * roll + D_rol * (roll - lastRoll)) + I_rol * roll_sum + P_pos * errorPosition_now[0] + D_pos * (errorPosition_now[0] - errorPosition_last[0]);
    // tmotor[1].vel_pd = -(P_pit * pitch + D_pit * (pitch - lastPitch)) + I_pit * pitch_sum + P_pos * errorPosition_now[1] + D_pos * (errorPosition_now[1] - errorPosition_last[1]);
    // tmotor[3].vel_pd = P_rol * roll + D_rol * (roll - lastRoll) + I_rol * roll_sum + P_pos * errorPosition_now[3] + D_pos * (errorPosition_now[3] - errorPosition_last[3]);
