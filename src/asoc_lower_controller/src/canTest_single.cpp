...
void rxThread(int s)
{
	...
	targetPositionMessage.data.resize(2);
    ...
    for (i = 0;; i++)
    {
		...
		if(i%4==0)
		{
			...
			printf("k kp ki are %f,%frun,%f",motor[0].K,motor[0].Kp,motor[0].Ki);
...
void controlV_calSendI_PD(int ID){
	int limit = 16667;
	motor[ID].velocityDifference = motor[ID].targetVelocity - motor[ID].velocity;
	if(motor[ID].velocityDifference > 1000) motor[ID].velocityDifference = 1000;
	if(motor[ID].velocityDifference < -1000) motor[ID].velocityDifference = -1000;
	...
	motor[ID].sendI = motor[ID].Kp*motor[ID].velocityDifference + motor[ID].Kd*motor[ID].velocity_diff;
...
void txThread(int s)
{
    ...
    for (int i = 0;; i++)
	{
		...
		if (flag ==1)
		{
            ...
            controlV_calSendI_PD(0);
			controlV_calSendI_PD(1);
			controlV_calSendI_PD(2);
			controlV_calSendI_PD(3);
