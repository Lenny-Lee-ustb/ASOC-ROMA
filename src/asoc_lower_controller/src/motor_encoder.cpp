...
int judge_forward=0;
int judge_backward=0;
int judge_left=0;
int judge_right=0;
int dir_forward = 0;
int dir_left = 0;

float forward_angle = 60.0;
...
void encoder_callback(std_msgs::Float32MultiArray encoder_msg){
    motor_low[0].encoder_angle = encoder_msg.data[0];
	motor_high[0].encoder_angle = encoder_msg.data[1];
	motor_low[2].encoder_angle = encoder_msg.data[2];
	motor_high[2].encoder_angle = encoder_msg.data[3];
...
void buttonCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
	...
	dir_forward = joy->buttons[7];
	dir_left = joy->buttons[6];
    for (int i=0; i<4; i++){
		if(judge_forward==1){
			if(i%2==0)
			{
...
std_msgs::Int32MultiArray positionMessage_low,positionMessage_high,velocityMessage_low,velocityMessage_high,IMessage_low,IMessage_high,sendIMessage_low,sendIMessage_high,targetPositionMessage_low,targetPositionMessage_high,targetVelocityMessage_low,targetVelocityMessage_high, encoder_msg;
...
void controlA_calSendI_PPI_high(int ID){
	int limit = 16667;
...
void control_encoder_speed_low(int ID){
	int limit = 16667;
	if(ID==0){
		motor_low[ID].angleDifference = motor_low[ID].targetAngle - motor_low[ID].encoder_angle;
		if(motor_low[ID].angleDifference > 120) motor_low[ID].angleDifference = 120;
		if(motor_high[ID].angleDifference < -120) motor_low[ID].angleDifference = -120;
		//motor_low[ID].angleDifferenceSum = motor_low[ID].angleDifference + motor_low[ID].angleDifferenceSum;

		motor_low[ID].targetVelocity = motor_low[ID].K_angle/100*(motor_low[ID].angleDifference);
		motor_low[ID].velocityDifference = motor_low[ID].targetVelocity - motor_low[ID].velocity;
		motor_low[ID].velocityDifferenceSum = motor_low[ID].velocityDifference + motor_low[ID].velocityDifferenceSum;
		...
		motor_low[ID].angleDifference = motor_low[ID].encoder_angle - motor_low[ID].targetAngle;
		if(motor_low[ID].angleDifference > 120) motor_low[ID].angleDifference = 120;
		if(motor_high[ID].angleDifference < -120) motor_low[ID].angleDifference = -120;
		//motor_low[ID].angleDifferenceSum = motor_low[ID].angleDifference + motor_low[ID].angleDifferenceSum;
...
void control_encoder_speed_high(int ID){
	int limit = 16667;
	if(ID==0){
		motor_high[ID].angleDifference = motor_high[ID].targetAngle - motor_high[ID].encoder_angle;
		if(motor_high[ID].angleDifference > 120) motor_high[ID].angleDifference = 120;
		if(motor_high[ID].angleDifference < -120) motor_high[ID].angleDifference = -120;
		//motor_low[ID].angleDifferenceSum = motor_low[ID].angleDifference + motor_low[ID].angleDifferenceSum;

		motor_high[ID].targetVelocity = motor_high[ID].K_angle/100*(motor_high[ID].angleDifference);
		motor_high[ID].velocityDifference = motor_high[ID].targetVelocity - motor_high[ID].velocity;
		motor_high[ID].velocityDifferenceSum = motor_high[ID].velocityDifference + motor_high[ID].velocityDifferenceSum;
		...
		motor_high[ID].angleDifference = motor_high[ID].encoder_angle - motor_high[ID].targetAngle;
		if(motor_high[ID].angleDifference > 120) motor_high[ID].angleDifference = 120;
		if(motor_high[ID].angleDifference < -120) motor_high[ID].angleDifference = -120;
		//motor_low[ID].angleDifferenceSum = motor_low[ID].angleDifference + motor_low[ID].angleDifferenceSum;
...
void txThread_low(int s)
{
    ...
    for (int i = 0;; i++)
	{
		...
		if (flag ==1 )
		{
			...
			control_encoder_speed_low(0);
			control_encoder_speed_low(1);
			//control_encoder_speed_low(2);
			//control_encoder_speed_low(3);
			
		}
		// else if(flag ==1 && dir_forward == 1 && dir_left == 0){
		// 	control_encoder_speed_low(0);
		// 	control_encoder_speed_low(1);
		// 	control_encoder_speed_low(2);
		// 	control_encoder_speed_low(3);
...
void txThread_high(int s)
{
    ...
    for (int i = 0;; i++)
	{
		...
		if (flag ==1)
		{
			...
			// controlP_calSendI_PI(2);
			// controlP_calSendI_PI(3);
			control_encoder_speed_high(0);
			control_encoder_speed_high(1);
			//control_encoder_speed_high(2);
			//control_encoder_speed_high(3);
		}
...
void orienThread(int forward, int backward, int left, int right){
