...
void buttonCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    for (int i=0; i<4; i++){
		...
		if(judge_forward==1){
			...
			// if(i%2==0)
			// {
			// 	motor_low[i].targetVelocity = 2800*stick_forward;
            // 	motor_high[i].targetVelocity = 2800*stick_forward;
			// }
			// else
			// {
			// 	motor_low[i].targetVelocity = -2800*stick_forward;
...
void body_to_wheel(float vt, float vn, float w){
    ...
    // wheel_high0 = motor_high[0].C.inverse()*vc_high0/r;
    // wheel_high2 = motor_high[2].C.inverse()*vc_high2/r;
...
int main(int argc, char** argv) {
    ...
    motor_low[0].ori_encoder = 16;
	motor_low[1].ori_encoder = 16;
	motor_low[2].ori_encoder = 275;
	motor_low[3].ori_encoder = 275;
	motor_high[0].ori_encoder = 88;
	motor_high[1].ori_encoder = 88;
	motor_high[2].ori_encoder = 52;
	motor_high[3].ori_encoder = 52;
